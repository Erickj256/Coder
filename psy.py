...
def conexion_rs(host_name, dbname, port, username, password):
    try: 
        ...
        print("Ocurrio un error al establecer la conexion") 
    else:
...
def actualizar_db(curr, data):
    temp_dataf = pd.DataFrame(columns = ['idclima', 'cityid', 'validdateutc', 'winddirectioncardinal',
                                        'probabilityofprecip', 'relativehumidity', 'name', 'date_insert',
                                        'longitude', 'state', 'lastreporttime', 'skydescriptionlong',
    ...
    for i, row in data.iterrows():
        ...
        #else:
            #temp_dataf = temp_dataf.append(row) #si no esta, el registro del clima, lo agregaremos a la tabla
